...
exports.deploy = async function() {
    ...
    console.log('** Deploy: bsc_test@20230907.js.js **');
    
    // ** Deploy: bbsc_test@20230710.js **
    // usdt: 0xDd4A68D8236247BDC159F7C5fF92717AA634cBCc
    // nest_old: 0x821edD79cc386E56FeC9DA5793b87a3A52373cdE
    // nest: 0xcd6926193308d3B371FdD6A6219067E550000000
    ...
    // pancakeRouter: 0xC293C13be5d5c669c638aC293ec9Adc9542ffEda
    // nestUsdtPair: 0x2Ac48df920d0676d2745C53FeE661e2F309b27c7
    // gatling: 0xB0BcACD079D0049B27Daa0a8cDA1D3979Ae5c5ca
    ...
    const nest = await NestToken.attach('0xcd6926193308d3B371FdD6A6219067E550000000');
    console.log('nest: ' + nest.address);

    const atf = await TestERC20.attach('0x000000fB6B0389cc3311198353A0b0f36AB03d44');
    console.log('atf: ' + atf.address);
    ...
    const pancakeRouter = await PancakeRouter.attach('0xC293C13be5d5c669c638aC293ec9Adc9542ffEda');
    console.log('pancakeRouter: ' + pancakeRouter.address);
    
    // await nest.mintTo('0x0e20201B2e9bC6eba51bcC6E710C510dC2cFCfA4', 500000000000000000000000000n);
    ...
    console.log({
        'pancake.pair.nestusdt': await nestGovernance.checkAddress('pancake.pair.nestusdt'),
        'nest': await nestGovernance.getNestTokenAddress()
    });
    ...
    await atf.approve(pancakeRouter.address, 1000000000000000000000000n);
    ...
    已经在bsc测试网上部署了新的NEST代币、空投用的批量转账合约、pancake兑换资金池以及新的期货合约

    1. 空投用的代币合约通过gatling.strafe(tokenAddress, targetAddresses, amounts, amountTotal)批量转账，转账时需要计算好代币总量并通过amountTotal传入
    2. pancake资金池使用的是新的NEST/USDT
    3. 新期货合约已经部署，但是没有更新，待准备好测试时再更新生效，生效后使用新NEST交易，原来的单子平仓时得到新NEST
    4. 去掉了nestVault合约，NEST和期货合约结算
