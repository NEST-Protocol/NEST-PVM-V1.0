...
namespace Baiynui.Utils
{
    ...
    public class Evaluate
    {
        ...
        public int calc(string expr)
        {
            ...
            Console.WriteLine($"{space(level)}@evaluate('{toString(expr, 0, expr.Length)}')={r}");
        ...
        void evaluatePart(
            ...
            int end,
            // result values
            out int index, out int cv, out int no
        )
        {
            //Console.WriteLine("evaluatePart: pv={2}, co=0x{3:x} [{0}], start={1}", toString(expr, start, end), start, pv, co);
            char c;
            int left = 0;
            int counter = 0;
            int state = S_NORMAL;
            ...
            while (index <= end)
            {
                ...
                else if (state == S_IDENTIFIER)
                {
                    ...
                    {
                        counter = (counter << 8) | (int)(c);
                    }
                    else
                    {
                        cv = (int)(_functionMap[counter]);
                        counter = 0;
                        state = S_OPERATOR;
                        continue;
                    }
                ...
                else if (state == S_BRACKET)
                {
                    ...
                    if (c == _RBR)
                    {
                        if (--counter == 0)
                        {
                            ...
                            cv = evaluate(level + 1, expr, left, index);

                            Console.WriteLine($"{space(level)}@evaluate('{toString(expr, left, index)}')={cv}");
                ...
                else if (state == S_OPERATOR)
                {
                    ...
                    if (c != _SPC)
                    {
                        // next operator, + - * / **
                        if (c == _ADD) { no = 0x1001; }
                        else if (c == _SUB) { no = 0x1002; }
                        else if (c == _MUL)
                        {
                            if (index + 1 < end && expr[index + 1] == _MUL) { ++index; no = 0x3001; }
                            else { no = 0x2001; }
                        }
                        else if (c == _DIV) { no = 0x2002; }
                ...
                else if (state == S_CALCULATE)
                {
                    ...
                    if ((no >> 8) > (co >> 8))
                    {
                        ...
                        evaluatePart(level + 1, cv, no, expr, index, end, out index, out cv, out no);
                        // now no is the last operator paraed by evaluatedPart just called

                        Console.WriteLine($"{space(level)}@evaluatePart({_cv}, 0x{_no:x}, '{toString(expr, _index, _end)}')= ({index}, {cv}, 0x{no:x})");

                    }

                    // Calculate with pv
                    if (co == 0x1001) { pv += cv; }
                    else if (co == 0x1002) { pv -= cv; }
                    else if (co == 0x2001) { pv = pv * cv; }
                    else if (co == 0x2002) { pv = pv / cv; }
                    else if (co == 0x3001) { pv = (int)(Math.Pow(pv, cv)); }
                    // co is 0, means this is the first part, pv = cv
                    else if (co == 0) { pv = cv; }

                    // no <= co, part end, break
                    if ((no >> 8) <= (co >> 8))
                    {
                        break;
                    }
