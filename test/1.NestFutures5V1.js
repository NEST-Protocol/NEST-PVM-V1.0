...
describe('1.NestFutures5V1.js', function() {
    it('First', async function() {
        var [owner, addr1, addr2] = await ethers.getSigners();
        
        const { eth, usdt, nest, nestFutures5V1, pancakeRouter, BLOCK_TIME } = await deploy();
        ...
        const listAccounts = async function(silent) {
            ...
            accounts = {
                ...
                nestVault: await listBalances(nestFutures5V1, tokens),
                nestFutures5V1: await listBalances(nestFutures5V1, tokens)
            };
        ...
        const post = async function(period, prices) {
            ...
            await nestFutures5V1.post(period, [toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2])]);
        ...
        const submit = async function(offlineOrders) {
            let orders = [];
            for (let i = 0; i < offlineOrders.length; ++i) {
                let offlineOrder = offlineOrders[i];
                orders.push({
                    // Owner of this order
                    //address owner;
                    owner: offlineOrder.owner,
                    // Balance of this order, 4 decimals
                    //uint40 balance;
                    balance: offlineOrder.balance * NEST_BASE,
                    // Index of target channel, support eth, btc and bnb
                    //uint8 channelIndex;
                    channelIndex: offlineOrder.channelIndex,
                    // Leverage of this order
                    //uint8 lever;
                    lever: offlineOrder.lever,
                    // Orientation of this order, long or short
                    //bool orientation;
                    orientation: offlineOrder.orientation,
                    // Base price of this order
                    //uint basePrice;
                    basePrice: toBigInt(offlineOrder.basePrice),
                    //uint40 fee;
                    fee: offlineOrder.fee * NEST_BASE,
                    // Stop price for trigger sell, encoded by encodeFloat40()
                    //uint stopProfitPrice;         // 56
                    stopProfitPrice: toBigInt(offlineOrder.stopProfitPrice),
                    // Stop price for trigger sell, encoded by encodeFloat40()
                    //uint stopLossPrice;           // 56
                    stopLossPrice: toBigInt(offlineOrder.stopLossPrice)
                });
            }
            await listAccounts(true);
            await nestFutures5V1.submit(orders);
            await listAccounts(true);
            ...
            for (let i = 0; i < offlineOrders.length; ++i) {
                ...
                ctx.orders.push({
                    ...
                    owner: offlineOrder.owner,
                    balance: offlineOrder.balance,
                    channelIndex: offlineOrder.channelIndex,
                    lever: offlineOrder.lever,
                    appends: 0,
                    orientation: offlineOrder.orientation,
                    basePrice: offlineOrder.basePrice,
                    openBlock: await bn(),
                    status: S_NORMAL,
                    fee: offlineOrder.fee,
                    stopProfitPrice: offlineOrder.stopProfitPrice,
                    stopLossPrice: offlineOrder.stopLossPrice
                });
            }
        };
        ...
        const execute = async function(period, prices, buyOrderIndices, sellOrderIndices, limitOrderIndices, stopOrderIndices, liquidateOrderIndices) {
            ...
            await nestFutures5V1.execute([toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2]), 0, 0, 0, 0], orderIndices
                // buyOrderIndices,
                // sellOrderIndices,
                // limitOrderIndices,
                // stopOrderIndices,
                ...
                if (order.status == S_BUY_REQUEST) {
                    if (order.orientation) {
                        if (order.basePrice >= oraclePrice && order.basePrice <= oraclePrice * (1 + SLIDING_POINT)) {
                            order.basePrice = oraclePrice * (1 + impactCost(order.balance * order.lever));
            ...
            for (var i = 0; i < sellOrderIndices.length; ++i) {
                ...
                if (order.status == S_SELL_REQUEST) {
                    ...
                    const miuT = 
                    order.channelIndex > 1 ?
                        (order.orientation
                            ? 5.787e-9 * (await bn() - order.openBlock) * BLOCK_TIME
                            :-5.787e-9 * (await bn() - order.openBlock) * BLOCK_TIME
                        ) :
                        (order.orientation
        ...
        const compareOrder = async function(index) {
            ...
            // Order from contract
            const co = UI((await nestFutures5V1.list(index, 1, 1))[0]);
        ...
        const balanceOf = async function(orderIndex, oraclePrice) {
            ...
            const cb = parseFloat(toDecimal(await nestFutures5V1.balanceOf(orderIndex, toBigInt(oraclePrice))));
        ...
        const add = async function(sender, index, amount) {
            await listAccounts(true);
            await nestFutures5V1.add(index, amount * NEST_BASE);
            await listAccounts(true);
        ...
        const newSellRequest = async function(sender, index, echo) {
            await nestFutures5V1.newSellRequest(index);
        ...
        const updateLimitPrice = async function(sender, orderIndex, limitPrice) {
            await nestFutures5V1.updateLimitPrice(orderIndex, toBigInt(limitPrice));
            let order = ctx.orders[orderIndex];
        ...
        const updateStopPrice = async function(sender, orderIndex, stopProfitPrice, stopLossPrice) {
            await nestFutures5V1.updateStopPrice(orderIndex, toBigInt(stopProfitPrice), toBigInt(stopLossPrice));
        ...
        const cancelBuyRequest = async function(sender, orderIndex) {
            await listAccounts(true);
            await nestFutures5V1.cancelBuyRequest(orderIndex);
            await listAccounts(true);
        ...
        const list = async function(sender, offset, count, order) {
            const cl = await nestFutures5V1.list(offset, count, order);
            const jl = [];
            if (order == 0) {
        ...
        await nestFutures5V1.deposit(toBigInt(10000));
        ...
        if (true) {
            ...
            await usdt.approve(nestFutures5V1.address, 10000000000000000000000000n);
            
            await listAccounts();
            await nestFutures5V1.newBuyRequestWithUsdt(
                //uint usdtAmount,
                toBigInt(10),
                //uint minNestAmount,
                toBigInt(100),
