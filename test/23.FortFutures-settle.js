...
describe('23.FortFutures-settle', function() {
    it('First', async function() {
        ...
        if (true) {
            ...
            await fortFutures.buy(eth.address, 5, true, toBigInt(10000), {
                value: toBigInt(0.01)
            });
            await fortFutures.buy(eth.address, 5, false, toBigInt(10000), {
                value: toBigInt(0.01)
            });
            await usdt.transfer(owner.address, 0);

            let l1 = await fortFutures.getFutureInfo(eth.address, 5, true);
            let l2 = await fortFutures.getFutureInfo(eth.address, 5, false);
