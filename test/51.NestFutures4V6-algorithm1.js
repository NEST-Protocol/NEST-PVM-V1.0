...
describe('51.nestFutures4V6-algorithm1.js', function() {
    it('First', async function() {
        var [owner, addr1, addr2] = await ethers.getSigners();
        
        const { eth, usdt, nest, nestFutures4V6, nestVault, gatling, pancakeRouter, BLOCK_TIME } = await deploy();
        ...
        const listAccounts = async function(silent) {
            ...
            accounts = {
                ...
                nestFutures4V6: await listBalances(nestFutures4V6, tokens)
            };
        ...
        await nest.approve(gatling.address, toBigInt(100000000));
        await gatling.strafe(nest.address, [nestVault.address, nestFutures4V6.address], [1234, 5678], 6911);
        await listAccounts();
        return;
        // Local context
        let ctx = {
        ...
        const post = async function(period, prices) {
            ...
            await nestFutures4V6.post(period, [toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2])]);
        ...
        const execute = async function(period, prices, buyOrderIndices, sellOrderIndices, limitOrderIndices, stopOrderIndices, liquidateOrderIndices) {
            ...
            await nestFutures4V6.execute([toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2]), 0, 0, 0, 0], orderIndices
                // buyOrderIndices,
                // sellOrderIndices,
                // limitOrderIndices,
                // stopOrderIndices,
        ...
        const compareOrder = async function(index) {
            ...
            // Order from contract
            const co = UI((await nestFutures4V6.list(index, 1, 1))[0]);
        ...
        const balanceOf = async function(orderIndex, oraclePrice) {
            ...
            const cb = parseFloat(toDecimal(await nestFutures4V6.balanceOf(orderIndex, toBigInt(oraclePrice))));
        ...
        const newBuyRequest = async function(sender, channelIndex, lever, orientation, amount, basePrice,
            ...
            await listAccounts(true);
            await nestFutures4V6.newBuyRequest(channelIndex, lever, orientation, amount * NEST_BASE, 
        ...
        const add = async function(sender, index, amount) {
            await listAccounts(true);
            await nestFutures4V6.add(index, amount * NEST_BASE);
            await listAccounts(true);
        ...
        const newSellRequest = async function(sender, index, echo) {
            await nestFutures4V6.newSellRequest(index);
        ...
        const updateLimitPrice = async function(sender, orderIndex, limitPrice) {
            await nestFutures4V6.updateLimitPrice(orderIndex, toBigInt(limitPrice));
            let order = ctx.orders[orderIndex];
        ...
        const updateStopPrice = async function(sender, orderIndex, stopProfitPrice, stopLossPrice) {
            await nestFutures4V6.updateStopPrice(orderIndex, toBigInt(stopProfitPrice), toBigInt(stopLossPrice));
        ...
        const cancelBuyRequest = async function(sender, orderIndex) {
            await listAccounts(true);
            await nestFutures4V6.cancelBuyRequest(orderIndex);
            await listAccounts(true);
        ...
        const list = async function(sender, offset, count, order) {
            const cl = await nestFutures4V6.list(offset, count, order);
            const jl = [];
            if (order == 0) {
        ...
        if (true) {
            ...
            await usdt.approve(nestFutures4V6.address, 10000000000000000000000000n);
            
            await listAccounts();
            await nestFutures4V6.newBuyRequestWithUsdt(
                //uint usdtAmount,
                toBigInt(10),
                //uint minNestAmount,
                toBigInt(100),
