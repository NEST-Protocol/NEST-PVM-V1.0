...
describe('HedgeOptions', function() {
    it('First', async function() {
        ...
        const { eth, usdt, dcu, hedgeOptions, hedgeFutures, nestPriceFacade, USDT_DECIMALS } = await deploy();
        ...
        const BLOCK = 100000;
        await hedgeOptions.open(eth.address, toBigInt(2450, USDT_DECIMALS), true, BLOCK, toBigInt(1000), {
            value: toBigInt(0.01)
        });
        ...
        await hedgeOptions.open(eth.address, toBigInt(2750, USDT_DECIMALS), false, BLOCK, toBigInt(1000), {
            value: toBigInt(0.01)
        });
