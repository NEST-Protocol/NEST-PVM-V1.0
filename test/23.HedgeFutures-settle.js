...
describe('HedgeOptions', function() {
    it('First', async function() {
        ...
        if (true) {
            ...
            await hedgeFutures.buy(eth.address, 5, false, toBigInt(10000), {
                value: toBigInt(0.01)
            });
            await usdt.transfer(owner.address, 0);

            let l1 = await hedgeFutures.getFutureInfo(eth.address, 5, true);
            let l2 = await hedgeFutures.getFutureInfo(eth.address, 5, false);
