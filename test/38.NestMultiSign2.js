...
describe('38.NestMultiSign2', function() {
    it('First', async function() {
        ...
        if (true) {
            console.log('2. sign');
            await nestMultiSign.connect(addr7).newTransaction(3, 1, nest.address, addr8.address, toBigInt(9527));
            ...
            await nestMultiSign.connect(addr2).executeTransaction(1, 0, 0);
            await listAccounts();
        }
        ...
        if (true) {
            ...
            expect(list[0].signs[1]).to.eq(addr2.address);
            expect(list[0].signs[2]).to.eq(addr4.address);
            expect(list[0].signs[3]).to.eq(addr7.address);
        }

        if (true) {
            console.log('3. newTransaction');
            await nestMultiSign.connect(addr3).newTransaction(1, 1, nest.address, addr8.address, toBigInt(1));
            let list = await nestMultiSign.list(0, 1, 0);
            console.log(UI(list));
            expect(list[0].signs[0]).to.eq(eth.address);
            expect(list[0].signs[1]).to.eq(addr3.address);
            expect(list[0].signs[2]).to.eq(eth.address);
            expect(list[0].signs[3]).to.eq(eth.address);
            await listAccounts();
        }
        ...
        if (true) {
            ...
            await nestMultiSign.connect(addr5).signTransaction(2, 1, 1);
            let list = await nestMultiSign.list(0, 1, 0);
            console.log(UI(list));
            expect(list[0].signs[0]).to.eq(addr1.address);
            expect(list[0].signs[1]).to.eq(addr3.address);
            expect(list[0].signs[2]).to.eq(addr5.address);
            expect(list[0].signs[3]).to.eq(eth.address);
            await listAccounts();
        }
        ...
        if (true) {
            ...
            await nestMultiSign.connect(addr6).executeTransaction(3, 0, 1);
            let list = await nestMultiSign.list(0, 1, 0);
            console.log(UI(list));
            expect(list[0].signs[0]).to.eq(addr1.address);
            expect(list[0].signs[1]).to.eq(addr3.address);
            expect(list[0].signs[2]).to.eq(addr5.address);
            expect(list[0].signs[3]).to.eq(addr6.address);
            await listAccounts();
            return;
        }
    });
