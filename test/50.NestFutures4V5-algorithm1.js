...
describe('49.nestFutures4V5-algorithm1.js', function() {
    it('First', async function() {
        ...
        const post = async function(period, prices) {
            ...
            await nestFutures4V5.post(period, [toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2])]);
        ...
        const execute = async function(period, prices, buyOrderIndices, sellOrderIndices, limitOrderIndices, stopOrderIndices, liquidateOrderIndices) {
            ...
            await nestFutures4V5.execute([toBigInt(prices[0]), toBigInt(prices[1]), toBigInt(prices[2]), 0, 0, 0, 0], orderIndices
                // buyOrderIndices,
                // sellOrderIndices,
                // limitOrderIndices,
                // stopOrderIndices,
        ...
        const compareOrder = async function(index) {
            ...
            // Order from contract
            const co = UI((await nestFutures4V5.list(index, 1, 1))[0]);
        ...
        const balanceOf = async function(orderIndex, oraclePrice) {
            ...
            const cb = parseFloat(toDecimal(await nestFutures4V5.balanceOf(orderIndex, toBigInt(oraclePrice))));
        ...
        const newBuyRequest = async function(sender, channelIndex, lever, orientation, amount, basePrice,
            ...
            await listAccounts(true);
            await nestFutures4V5.newBuyRequest(channelIndex, lever, orientation, amount * NEST_BASE, 
        ...
        const add = async function(sender, index, amount) {
            await listAccounts(true);
            await nestFutures4V5.add(index, amount * NEST_BASE);
            await listAccounts(true);
        ...
        const newSellRequest = async function(sender, index, echo) {
            await nestFutures4V5.newSellRequest(index);
        ...
        const updateLimitPrice = async function(sender, orderIndex, limitPrice) {
            await nestFutures4V5.updateLimitPrice(orderIndex, toBigInt(limitPrice));
            let order = ctx.orders[orderIndex];
        ...
        const updateStopPrice = async function(sender, orderIndex, stopProfitPrice, stopLossPrice) {
            await nestFutures4V5.updateStopPrice(orderIndex, toBigInt(stopProfitPrice), toBigInt(stopLossPrice));
        ...
        const cancelBuyRequest = async function(sender, orderIndex) {
            await listAccounts(true);
            await nestFutures4V5.cancelBuyRequest(orderIndex);
            await listAccounts(true);
        ...
        const list = async function(sender, offset, count, order) {
            const cl = await nestFutures4V5.list(offset, count, order);
            const jl = [];
            if (order == 0) {
        ...
        if (true) {
            ...
            await usdt.approve(nestFutures4V5.address, 10000000000000000000000000n);
