...
describe('23.NestFutures-settle-hbtc', function() {
    it('First', async function() {
        ...
        if (true) {
            ...
            await nestFutures.buy(hbtc.address, 5, true, toBigInt(10000), {
                value: toBigInt(0.01)
            });
            await nestFutures.buy(hbtc.address, 5, false, toBigInt(10000), {
                value: toBigInt(0.01)
            });
            await usdt.transfer(owner.address, 0);

            let l1 = await nestFutures.getFutureInfo(hbtc.address, 5, true);
            let l2 = await nestFutures.getFutureInfo(hbtc.address, 5, false);
