...
contract NestFutures4V6 is NestFrequentlyUsed, INestFutures4 {
    ...
    /// @param prices Price array, direct price, eth&btc&bnb&matic&ada&doge&xrp, eg: 1700e18, 25000e18, 300e18
    ...
    function execute(uint[CHANNEL_COUNT] calldata prices, uint[] calldata orderIndices) external {
        ...
        while (i > 0) {
            ...
            uint oraclePrice = prices[uint(order.channelIndex)];

            // 1. Execute buy request
            if (status == S_BUY_REQUEST) {
                uint basePrice = CommonLib.decodeFloat(uint(order.basePrice));

                // basePrice not match oraclePrice, revert
    ...
    function newBuyRequest(
        ...
        TransferHelper.safeTransferFrom(
            NEST_TOKEN_ADDRESS, 
            msg.sender, 
            address(this), //NEST_VAULT_ADDRESS, 
            (
                amount + 
    ...
    function newBuyRequestWithUsdt(
        ...
        uint nestAmount = _swapUsdtForNest(usdtAmount, minNestAmount, address(this)/*NEST_VAULT_ADDRESS*/);
