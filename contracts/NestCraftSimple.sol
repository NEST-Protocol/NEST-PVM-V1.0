...
contract NestCraftSimple is NestCraft {
    ...
    function _evaluatePart(
        ...
        for (; ; )
        {
            ...
            if (state == S_NORMAL)
            {
                ...
                {
                    revert("NC:bracket expression not support");
                    // unchecked {
            ...
            // identifier
                ...
                // Identifier end
                    ...
                    revert("NC:identifier not support");
            ...
            // function
                ...
                unchecked {
                    ...
                    if (c == $CMA && cv == 1) {
                        ...
                        revert("NC:nested expression not support");
                        //++cv;
                    } 
                }
            }
            ...
            else if (state == S_OPERATOR)
            {
                ...
                if (c != $SPC)
                {
                    unchecked {
                        ...
                        if (c == $SUB) { revert("NC:sub not support"); /* co = 0x1002; */ } else 
                        if (c == $MUL) {
                        ...
                        if (c == $DIV) { revert("NC:div not support"); /* co = 0x2002; */ } else 
                        // eof, next operator
                        if (c == $EOF) { co = 0x0001; }
                        else { revert("PVM:expression invalid"); }
